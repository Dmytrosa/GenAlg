...
export const makeInitialPopulation = (count = 20) => {
    return [...new Array(count)].map(() => {
        const lections = weeklyLecturesPlan.map((e) => {
            ...
            return initializeLection(Object.assign(Object.assign({}, e), { lecturer }));
        });
        return new TimeTable(lections);
    });
};
const initializeLection = (e) => new Lecture(e.science, getRandomItem(timings), getRandomItem(auditories), getRandomItem(lecturers), e.type, e.group, getRandomItem(days), e.maxStudents);
